...
void CMesh::Init(CModelX* model)
{
	...
	mpAnimationVertex = new CVector[mVertexNum];
	...
	while (!model->EOT())
	{
		...
		if (strcmp(model->Token(), "MeshNormals") == 0)
		{
			...
			mpAnimationNormal = new CVector[mNormalNum];
	...
	while (!model->EOT())
	{
		...
		if (strcmp(model->Token(), "AnimationKey") == 0)
		{
			...
			case 4://行列データを取得
				...
				for (int i = 0; i < mKeyNum;i++)
				{
					mpKey[i].mTime = atof(model->GetToken());
					model->GetToken();
					for (int j = 0;j < 16;j++)
					{
...
void CAnimationSet::AnimateMatrix(CModelX* model)
{
	...
	for (size_t j = 0;j < mAnimation.size();j++)
	{
		...
		else
		{
			...
			{
				//変換行列を、線形補間にて更新
				if (mTime < animation->mpKey[k].mTime && animation->mpKey[k - 1].mTime != animation->mpKey[k].mTime)
				{
					float r = (animation->mpKey[k].mTime - mTime) / (animation->mpKey[k].mTime - animation->mpKey[k - 1].mTime);

					frame->mTransformMatrix += (animation->mpKey[k - 1].mMatrix * r + animation->mpKey[k].mMatrix * (1 - r)) * mWeight;
					break;
				}
			}
		}
...
void CMesh::SetSkinWeighFrameIndex(CModelX* model)
{
    //スキンウェイト分繰り返し
	for (size_t i = 0;i < mSkinWeights.size();i++)
	{
...
void CModelX::SetSkinWeightFrameIndex()
{
	...
	for (size_t i = 0;i < mFrame.size();i++)
	{
		...
		if (mFrame[i]->mpMesh != nullptr)
		{
			mFrame[i]->mpMesh->SetSkinWeighFrameIndex(this);
		}
	}
}
...
{
	//アニメーション用の頂点エリアクリア
	memset(mpAnimationVertex, 0, sizeof(CVector) * mVertexNum);
	memset(mpAnimationNormal, 0, sizeof(CVector) * mNormalNum);
	...
	for (size_t i = 0;i < mSkinWeights.size();i++)
	{
		...
		for (int j = 0;j < mSkinWeights[i]->mIndexNum;j++)
		{
			...
			//頂点と法線を更新する
			mpAnimationVertex[index] += mpVertex[index] * mSkinningMatrix * weight;
			mpAnimationNormal[index] += mpNormal[index] * mSkinningMatrix * weight;
		}
	}
	//法線を正規化する
	for (int i = 0;i < mNormalNum;i++)
...
void CModelX::SeparateAnimationSet(int idx, int start, int end, char* name)
{
	...
	for (size_t i = 0;i < anim->mAnimation.size();i++)
	{
		...
		as->mAnimation.push_back(animation); //アニメーションセットの追加
	}
